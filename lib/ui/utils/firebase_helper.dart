...
class FirebaseHelper {
  ...
  Future<User?> handleCreate(
      String mail, String password, String prenom, String nom) async {
    final UserCredential result = await auth.createUserWithEmailAndPassword(
        email: mail, password: password);
    final User? user = result.user;
    String uid = user!.uid;
    Map<String, String> map = {"uid": uid, "prenom": prenom, "nom": nom};
    addUser(uid, map);
    return user;
  }
  ...
  sendMessage(User user, User moi, String text, String imageUrl) {
    ...
    base_conversation
        .child(moi.id)
        .child(user.id)
        .set(getConversation(moi.id, user, text, date));
    base_conversation
        .child(user.id)
        .child(moi.id)
        .set(getConversation(moi.id, moi, text, date));
  }
